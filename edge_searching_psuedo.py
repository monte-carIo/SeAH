...
def sliding_window(img, w_size = W_SIZE, debug = True, camRight=True):
    ...
    img_draw = img.copy()
    if not camRight:
        res = findStartingPoint(mask_img_draw, canny_img, img_draw, 15,50, startY=.95)
    else:
        res = findStartingPoint(mask_img_draw, canny_img, img_draw)
    if res is None:
        return [], []
    ...
    for _ in range(100):
        ...
        if len(inner_segs) and len(outer_segs): #TODO: or , not and
            ...
            # filter_idx= np.where(((np.asarray(dist2outer) < 3) | (np.asarray(dist2inner) < 3)))
                ...
                filter_idx = np.where(((angles_outer-prev_angle_outer > 1) & (angles_outer-prev_angle_outer < MIN_ANGLE_DIFF))
                    | ((angles_inner-prev_angle_inner > 1) & (angles_inner-prev_angle_inner < MIN_ANGLE_DIFF)) ) # TODO - bright intensity change a little, FIXME: 2 detected lines are shifted but 1 close to inner/outer
...
if __name__ == '__main__':
    ...
    inner_segs, outer_segs = sliding_window(img, debug=True, camRight=False) #FIXME - stop without median condition
